...
## آخر التحديثات والتنبيه الاستراتيجي (2026-04-23)

### تحديث صارم لمعالجة التنبيهات وسطح الطلبات (2026-04-23)

- تمت معالجة التبعيات المتأثرة فعليًا عبر ترقية `actix-http` إلى `3.12.1` و`rustls-webpki` إلى `0.103.13` دون توسيع غير ضروري لسطح الاعتماديات.
- تمت إضافة رفض مركزي صارم للطلبات ذات التغليف الملتبس عند اجتماع `Content-Length` مع `Transfer-Encoding` لحماية الواجهة من أنماط CL.TE request smuggling.
- لم يعد النظام يثق في `X-Forwarded-For` افتراضيًا، وبذلك لا يمكن تجاوز تحديد المعدل أو الحظر التكيفي عبر spoofing إلا إذا فُعّل ذلك صراحةً بسياسة تشغيلية (`TRUST_X_FORWARDED_FOR`).
- تم تقليل بقايا المفتاح السري في الذاكرة عبر تخزين مادة توقيع JWT داخل حاوية تمسح الذاكرة تلقائيًا.
- تمت إضافة اختبارات صارمة لمسارات request smuggling، ومقاومة spoofing، وتشذيب حالة AI guard لمنع الاحتفاظ غير المنضبط بالذاكرة.
- تمت مراجعة سطح الويب هوك ضمن نفس الجولة، ولم يُعثر على أي webhook endpoints مسجلة في الواجهة الحالية، لذا يبقى هذا السطح غير مفعّل تصميميًا في خط الأساس النشط.

### تحديث حوكمة الاعتمادات ومنع تكرار الإشكاليات (2026-04-15)

- تم تنفيذ جولة صيانة أمنية صارمة لإزالة المسارات الاعتمادية عالية المخاطر من خط الأساس التشغيلي.
- سبب بدء التعديل كان تنبيهات GitHub الأمنية (Dependabot/Code Scanning) التي أظهرت انحرافًا أمنيًا وأخطاء تحليل بناء تراكمية.
- تمت معالجة مسار الاستبدال بشكل صريح: إزالة `jsonwebtoken` (وما يتبعه) لصالح HS512 داخلي، واستبدال `ammonia` بـ `html-escape` لتقليل المسارات الاعتمادية عالية الحساسية.
- تم توثيق الحدث بصيغة تحليل علمي تتضمن: المشكلة، السبب الجذري، الإجراءات التصحيحية، والضوابط الوقائية طويلة المدى.
- تم تعزيز التنظيف التشغيلي للمستودع لإزالة الملفات المؤقتة/العشوائية ومخلفات البناء التي قد تؤثر على جودة التحقق.
- تم إغلاق طلبات السحب القديمة غير المتزامنة مع `main` عندما كانت تفشل الفحوصات الإلزامية، حفاظًا على اتساق خط الدمج.
